...
**Compétence visée** : **C2.3.2** — _Élaborer un plan de correction des bogues à partir de l'analyse des anomalies et des régressions détectées au cours de la recette afin de garantir le fonctionnement du logiciel conformément à l'attendu._
...
**Règle de non-régression** : _toute anomalie corrigée doit être accompagnée d'un test automatisé qui échouait avant le correctif et passe après._ Les seuils de couverture configurés dans `vite.config.ts` empêchent une régression silencieuse de la couverture.
