<script lang="ts">
	...
	// Writable derived: defaults to the booking named in the URL (or the most recent one)
	// and stays bindable to the <Select> below.
	let selectedBookingId = $derived(data.selectedBookingId || (hostBookings[0]?.id ?? ''));
	let damageType = $state('SOUND_SYSTEM');
	let description = $state('');
	let estimatedCost = $state<number | undefined>(undefined);
	let loading = $state(false);
	let errorMessage = $state('');
	let claimSuccess = $state<ClaimResult | null>(null);
	...
	const selectedBooking = $derived(hostBookings.find((b) => b.id === selectedBookingId));
...
<div class="mx-auto max-w-3xl space-y-8 px-4 py-10 sm:px-6 lg:px-8">
	...
	<div class="space-y-2 text-center">
		...
		<p class="text-xs text-slate-500">
			Protection Wakam jusqu'à 10 000 € pour les hôtes mettant leur bien à disposition. Fenêtre de
			déclaration : 7 jours post-événement.
		</p>
	</div>

	<!-- RBAC Info Banner -->
	<div
		class="space-y-1.5 rounded-xl border border-purple-200 bg-purple-50/60 p-4 text-xs text-purple-950"
	>
		...
		<p class="leading-relaxed">
			La déclaration de sinistre est <strong>strictement réservée aux propriétaires (Hôtes)</strong> ayant
			mis leur bien à disposition. Les convives/locataires ne peuvent pas déclarer de sinistre. En tant
			que convive, vous disposez d'un droit de contestation avec pièces justificatives depuis votre tableau
			de bord.
		</p>
	</div>
		...
		<Card class="space-y-6 border-emerald-300 bg-emerald-50/50 p-8 text-center">
			...
			<div class="space-y-2">
				...
				<p class="text-xs text-slate-600">
					Dossier N° <strong class="font-bold text-emerald-800">{claimSuccess.claimNumber}</strong>
					rattaché à la police
					<strong class="font-bold text-slate-900">{claimSuccess.policyNumber}</strong>.
				</p>
			</div>

			<div
				class="space-y-2 rounded-xl border border-emerald-200 bg-white p-4 text-left text-xs text-slate-700"
			>
			...
			<div class="flex flex-wrap justify-center gap-3 pt-2">
				<Button
					href={`/bookings/${claimSuccess.bookingId}/certificate`}
					target="_blank"
					variant="outline"
					class="gap-2"
				>
				...
				<Button href="/dashboard" variant="default">Retourner à mon Espace</Button>
			</div>
		</Card>
	{:else}
		{#if errorMessage}
			...
			<form onsubmit={handleSubmit} class="space-y-6">
				...
				<div class="flex flex-col gap-1.5">
					...
					{#if hostBookings.length === 0}
						<p class="rounded-lg border border-rose-200 bg-rose-50 p-3 text-xs text-rose-600">
							Aucune réservation trouvée pour laquelle vous êtes l'hôte propriétaire. Seul le
							propriétaire du logement mis à disposition peut déclarer un sinistre.
						</p>
					{:else}
						<Select.Root type="single" name="bookingId" bind:value={selectedBookingId}>
							<Select.Trigger class="w-full">
								{selectedBooking
									? `${selectedBooking.listing.title} (${selectedBooking.listing.city}) — Invité: ${selectedBooking.guest?.firstName} ${selectedBooking.guest?.lastName}`
								...
								<Select.Group>
									...
									{#each hostBookings as b (b.id)}
										<Select.Item value={b.id} label={b.listing.title}>
											{b.listing.title} ({b.listing.city}) — Invité : {b.guest?.firstName}
											{b.guest?.lastName}
										</Select.Item>
									{/each}
								</Select.Group>
				...
				{#if isNotEndedYet}
					<div
						class="rounded-lg border border-amber-200 bg-amber-50 p-3 text-center text-xs font-medium text-amber-800"
					>
						L'événement n'est pas encore terminé. La déclaration de sinistre sera disponible dès la
						fin de la réservation.
					</div>
				{:else if isWindowExpired && selectedBooking}
					<div
						class="rounded-lg border border-rose-200 bg-rose-50 p-3 text-center text-xs font-medium text-rose-800"
					>
						La fenêtre de déclaration de sinistre de 7 jours post-événement est expirée pour cette
						réservation.
					</div>
				{/if}
