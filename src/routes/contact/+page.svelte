...
<div class="mx-auto max-w-4xl space-y-10 px-4 py-12 sm:px-6 lg:px-8">
	...
	<div class="space-y-3 text-center">
		...
		<p class="mx-auto max-w-xl text-xs text-slate-500">
			Une question sur une réservation, la garantie Wakam ou la mise en ligne de votre bien ? Nos
			conseillers vous répondent sous 24h.
		</p>
	</div>
	...
	<div class="grid grid-cols-1 gap-8 md:grid-cols-3">
		...
		<div class="space-y-4 md:col-span-1">
			<Card.Root class="space-y-2 border-slate-200 p-4">
				<div class="flex items-center gap-2 text-xs font-bold text-purple-900">
					<Mail class="h-4 w-4 text-purple-700" />
					Par Email
				</div>
				<p class="text-xs font-semibold text-slate-600">support@evenue.fr</p>
			</Card.Root>
			...
			<Card.Root class="space-y-2 border-slate-200 p-4">
				...
				<p class="text-xs font-semibold text-slate-600">+33 (0)1 89 40 12 34</p>
			</Card.Root>
		...
		<!-- Contact Form -->
		<Card.Root class="space-y-6 border-slate-200 p-6 md:col-span-2">
			{#if sent}
				<div class="space-y-3 py-8 text-center">
					...
					<p class="text-xs text-slate-600">
						Notre équipe support prend en charge votre demande et vous répondra à l'adresse <strong
							>{email}</strong
			...
			{:else}
				<form onsubmit={handleSubmit} class="space-y-4">
					...
					<div class="space-y-1.5">
						...
						<InputGroup.Root>
							<InputGroup.Input
								...
								type="text"
								bind:value={subject}
								placeholder="Question sur ma réservation, l'assurance Wakam..."
								required
							/>
						</InputGroup.Root>
					</div>
					...
					<Button
						type="submit"
						variant="default"
						class="w-full gap-2 bg-purple-900 font-bold text-white hover:bg-purple-800"
					>
						<Send class="h-4 w-4" />
						Envoyer mon message
					</Button>
