<script lang="ts">
	...
	// Writable derived: follows the server-rendered count, and the 15 s poll below can
	// still overwrite it until the next navigation refreshes the prop.
	let unreadCount = $derived(unreadMessageCount);
...
<header class="sticky top-0 z-50 border-b border-slate-200 bg-white transition-all">
	<div class="mx-auto flex h-20 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
		...
		<div class="flex items-center gap-3">
			{#if user}
				...
				<DropdownMenu.Root>
					<DropdownMenu.Trigger
						class="relative inline-flex h-9 items-center justify-center gap-2.5 rounded-lg border border-slate-200 bg-white px-2.5 text-sm font-medium text-slate-900 transition-all hover:bg-slate-50 focus-visible:ring-3 focus-visible:ring-slate-300 focus-visible:outline-none"
					...
					>
						...
						{#if unreadCount > 0}
							<span
								class="absolute -top-2 -right-2 flex h-5 min-w-5 items-center justify-center rounded-full border-2 border-white bg-rose-600 px-1 text-[10px] leading-none font-bold text-white"
								aria-hidden="true"
							>
						...
						<DropdownMenu.Group>
							{#if user.role === 'HOST'}
								<DropdownMenu.Item class="p-0">
									<a
										href="/listings/new"
										class="flex w-full items-center gap-2 px-2 py-1.5 font-medium text-slate-700"
									>
										<CirclePlus />
							...
							<DropdownMenu.Item class="p-0">
								...
								>
									...
									{#if unreadCount > 0}
										<span
											class="flex h-5 min-w-5 items-center justify-center rounded-full border-2 border-white bg-rose-600 px-1 text-[10px] leading-none font-bold text-white"
			...
			{:else}
				...
				<a
					href="/become-host"
					class="hidden items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-700 transition-colors hover:bg-slate-50 sm:inline-flex"
				>
				...
				<a
					href="/auth/login"
					class="inline-flex items-center gap-2 rounded-lg bg-slate-950 px-3 py-2 text-sm font-medium text-white transition-colors hover:bg-slate-800"
				>
					<User class="h-4 w-4" />
					Se connecter
				</a>
